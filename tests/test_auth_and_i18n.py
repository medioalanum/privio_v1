...
def test_branded_login_and_session_flow(unauth_client: TestClient) -> None:
    ...
    invalid = unauth_client.post("/login", data={"role": "admin", "password": "wrong"})
...
def test_auth_client_role_permissions(
    readonly_client: TestClient, admin_client: TestClient
) -> None:
    """Test that Client role can read data but cannot mutate (403 Forbidden)."""
    # Create a commitment with admin
    ...
    # Client can read commitments, upcoming, suggested-monthly, reserve-balance, deposits, dashboard
    assert readonly_client.get("/commitments").status_code == 200
    assert readonly_client.get(f"/commitments/{item_id}").status_code == 200
    assert readonly_client.get("/upcoming?days=30").status_code == 200
    assert readonly_client.get("/suggested-monthly").status_code == 200
    assert readonly_client.get("/deposits").status_code == 200
    assert readonly_client.get("/reserve-balance").status_code == 200
    assert readonly_client.get("/").status_code == 200
    assert readonly_client.get("/ui/upcoming?days=30").status_code == 200

    # Client CANNOT create commitment (403)
    res_post = readonly_client.post(
        "/commitments",
        json={
    ...
    # Client CANNOT update commitment (403)
    res_put = readonly_client.put(
    ...
    # Client CANNOT patch commitment (403)
    res_patch = readonly_client.patch(
        f"/commitments/{item_id}",
        json={"amount": "130.00"},
    )
    assert res_patch.status_code == 403

    # Client CANNOT delete commitment (403)
    res_del = readonly_client.delete(f"/commitments/{item_id}")
    assert res_del.status_code == 403

    # Client CANNOT create deposit (403)
    res_dep = readonly_client.post(
        "/deposits",
...
def test_auth_admin_role_full_access(admin_client: TestClient) -> None:
    """Test that Admin role has full access to create, read, update, and delete."""
    # Create
    create_res = admin_client.post(
        "/commitments",
        json={
    ...
    # Delete
    del_res = admin_client.delete(f"/commitments/{item_id}")
    assert del_res.status_code == 204


def test_i18n_dashboard_and_partials(readonly_client: TestClient) -> None:
    """Test dictionary-based i18n support for pt, en, and it."""
    # 1. Portuguese (default)
    res_pt = readonly_client.get("/?lang=pt")
    ...
    # 2. English
    res_en = readonly_client.get("/?lang=en")
    ...
    # 3. Italian
    res_it = readonly_client.get("/?lang=it")
    ...
    partial_en = readonly_client.get("/ui/upcoming?days=30&lang=en")
    assert partial_en.status_code == 200
    assert "Remaining payments" in partial_en.text
    assert "Due Date" in partial_en.text

    partial_it = readonly_client.get("/ui/upcoming?days=60&lang=it")
    ...
    res_fallback = readonly_client.get("/?lang=fr")
